...
export class TmdbClient {
  ...
  async search(query: string, limit = 10): Promise<MediaSummary[]> {
    ...
    return (data.results ?? [])
      ...
      .slice(0, limit)
      .map((item) => toSummary(item, item.media_type === 'tv' ? MediaType.TV : MediaType.MOVIE));
  }
...
function toSummary(item: TmdbSearchItem, mediaType: MediaType): MediaSummary {
  ...
  return {
    ...
    episodeCount: mediaType === MediaType.TV ? (detail.number_of_episodes ?? null) : null,
    runtimeMinutes: mediaType === MediaType.MOVIE ? (detail.runtime ?? null) : null,
    seasonCount: mediaType === MediaType.TV ? (detail.number_of_seasons ?? null) : null,
  };
}

...
import { buildGuildPage, type GuildPageData, type TitleTally } from '@/lib/services/guild';
      ...
      <main className="mx-auto max-w-5xl px-5 pb-20">
        <div className="mt-8 flex items-center gap-4">
          {page.guild.iconUrl ? (
            ...
            <img src={page.guild.iconUrl} alt="" className="h-14 w-14 rounded-2xl border border-border object-cover" />
          ) : (
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h1 className="font-display text-2xl font-bold">{page.guild.name}</h1>
            ...
            <p className="muted mt-1">
              {page.memberCount} Couchlist member{page.memberCount === 1 ? '' : 's'} · server taste perks unlocked
            </p>
          </div>
        </div>

        <div className="mt-6 rounded-2xl border border-primary/20 bg-primary/[0.07] px-4 py-4">
          <p className="font-display font-bold text-[#c9d3ff]">What this server unlocks ✦</p>
          <p className="muted mt-1">
            Members can see what the community is watching, its favorite titles, what everyone wants
            to watch next, and discover other Couchlist users from this server.
          </p>
        </div>

        {page.members.length > 0 ? (
          <section className="mt-10">
            <h2 className="font-display mb-4 text-lg font-bold">Members on Couchlist</h2>
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {page.members.slice(0, 9).map((member) => (
                <Link
                  key={member.id}
                  href={member.id === user.id ? '/profile' : `/profile/${member.id}`}
                  className="card flex items-center gap-3 px-4 py-3 transition hover:-translate-y-0.5 hover:border-primary/50"
                >
                  {member.avatarUrl ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={member.avatarUrl} alt="" className="h-10 w-10 rounded-full border border-border object-cover" />
                  ) : (
                    <span className="flex h-10 w-10 items-center justify-center rounded-full border border-border bg-background text-xs font-bold">
                      {(member.globalName ?? member.username).slice(0, 1).toUpperCase()}
                    </span>
                  )}
                  <div className="min-w-0">
                    <p className="truncate text-sm font-bold">{member.globalName ?? member.username}</p>
                    <p className="muted truncate">{member.watching ? `Watching ${member.watching}` : 'See their taste'}</p>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <TallySection title="Currently Watching" items={page.currentlyWatching} unit="watching" />
        <TallySection title="Highest Rated" items={page.highestRated} showRating />
        <TallySection title="Want to Watch" items={page.wantToWatch} unit="interested" />
...
async function loadGuildPageData(guildId: string): Promise<GuildPageData | null> {
  try {

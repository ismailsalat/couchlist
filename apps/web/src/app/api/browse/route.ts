...
export const GET = apiRoute(async (request) => {
  ...
  if (!Number.isInteger(pageValue) || pageValue < 1 || pageValue > MAX_BROWSE_PAGES) {
    throw AppError.validation(`Browse page must be between 1 and ${MAX_BROWSE_PAGES}.`);
  }

  return getBrowsePage(filter, sort, pageValue);
});

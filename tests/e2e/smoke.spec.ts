...
async function devLogin(
  page: Page,
  account: { discordId: string; username: string },
) {
...
test.describe("landing", () => {
  test("shows the tagline and a Discord sign-in button", async ({ page }) => {
    ...
    await expect(
      page.getByRole("link", { name: /continue with discord/i }),
    ).toBeVisible();
  });
...
test.describe("signed in", () => {
  ...
  test("home shows friends and popular sections", async ({ page }) => {
    await page.goto("/home");
    await expect(
      page.getByRole("heading", { name: "Friends Watching" }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Popular With Friends" }),
    ).toBeVisible();
  });
  ...
  test("search finds a title", async ({ page }) => {
    ...
    await expect(
      page.getByRole("search").or(page.locator('input[type="search"]')),
    ).toBeVisible();
  });
  ...
  test("profile shows the three status counts", async ({ page }) => {
    ...
    await expect(
      page.getByText("Plan to Watch", { exact: true }),
    ).toBeVisible();
  });
  ...
  test("title page shows friend-circle statistics", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "With Your Friends" }),
    ).toBeVisible();
  });

  test("title status can be changed repeatedly without getting stuck", async ({
    page,
  }) => {
    await page.goto("/media/anilist/anime/16498");

    const watching = page.getByRole("button", {
      name: "Watching",
      exact: true,
    });
    ...
    await expect(page.getByText(/Watching saved/i)).toBeVisible();

    await plan.click();
    await expect(plan).toHaveAttribute("aria-pressed", "true");
    await expect(page.getByText(/Plan to Watch saved/i)).toBeVisible();

    // Restore the seeded state so later smoke tests see the normal fixture.
  ...
  test("server page loads", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Test Server" }),
    ).toBeVisible();
  });
  ...
  test("watch together page loads", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /find something to watch/i }),
    ).toBeVisible();
  });
...
test.describe("authorization", () => {
  test("an unknown user cannot be read through comparison", async ({
    page,
  }) => {
    await devLogin(page, TEST_USER);

...
import { aniListSearchFixture, tmdbSearchFixture, tmdbMovieFixture } from '../helpers/fixtures';
...
describe('AniList adapter', () => {
  ...
  it('times out rather than hanging', async () => {
    ...
    const impatient = new AniListClient({ apiUrl: 'https://graphql.anilist.co', timeoutMs: 20 });
...
describe('TMDB adapter', () => {
  const client = new TmdbClient({ apiKey: 'test-key', baseUrl: 'https://api.themoviedb.org/3' });
  ...
  it('maps movie detail', async () => {
    ...
    expect(detail).toMatchObject({ runtimeMinutes: 88, genres: ['Science Fiction'] });
  });

  it('never puts the api key in the url when a bearer token is used', async () => {
    const spy = vi.fn(async () => new Response(JSON.stringify(tmdbSearchFixture), { status: 200 }));
    globalThis.fetch = spy as unknown as typeof fetch;

    const v4 = new TmdbClient({ apiKey: 'header.token.value', baseUrl: 'https://api.themoviedb.org/3' });
    await v4.search('x');
  ...
  it('returns nothing when no api key is configured', async () => {
    ...
    const unconfigured = new TmdbClient({ apiKey: '', baseUrl: 'https://api.themoviedb.org/3' });
  ...
  it('surfaces a provider error on failure', async () => {
    ...
    await expect(client.search('x')).rejects.toMatchObject({ code: 'CL_MEDIA_PROVIDER_ERROR' });
  });
});
